...
class FancyArrow3D(FancyArrowPatch):
    ...
    def draw(self, renderer):
        ...
        # 获取投影矩阵，优先使用axes.M，如果不存在则使用单位矩阵
        M = getattr(self.axes, 'M', np.eye(4)) if hasattr(self, 'axes') else np.eye(4)
        xs, ys, zs = proj3d.proj_transform(xs3d, ys3d, zs3d, M)
        self.set_positions((xs[0],ys[0]),(xs[1],ys[1]))
        FancyArrowPatch.draw(self, renderer)

    def do_3d_projection(self, renderer=None):
        """为最新版本的Matplotlib添加3D投影支持"""
        if hasattr(self, 'axes') and hasattr(self.axes, 'M'):
            # 如果axes和M矩阵可用，使用它们
            xs3d, ys3d, zs3d = self._verts3d
            xs, ys, zs = proj3d.proj_transform(xs3d, ys3d, zs3d, self.axes.M)
            self.set_positions((xs[0], ys[0]), (xs[1], ys[1]))
            return min(zs)
        elif renderer and hasattr(renderer, 'M'):
            # 否则尝试使用renderer的M矩阵
            ...
            import numpy as np
            xs3d, ys3d, zs3d = self._verts3d
            xs, ys, zs = proj3d.proj_transform(xs3d, ys3d, zs3d, np.eye(4))
            self.set_positions((xs[0], ys[0]), (xs[1], ys[1]))
            return min(zs)
